...
void setup() {
  ...
  for (int i = 0; i < 5; i ++) {
    ...
    switch (i) {
      case 0: colour = color(#ff0000); 
        break;
      case 1: colour = color(#ffff00);
        break;
      case 2: colour = color(#00ff00);
        break;
      case 3: colour = color(#00ffff);
        break;
      case 4: colour = color(#0000ff);
        break;
    };
...
void settingsMenu() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    ellipse((i+1)*width/6, 4.5*height/12, height/12, height/12);
    fill(255);
    ...
    if (planetPlaceholders[i].selected == false) {
      fill(0,0,0,200);
      noStroke();
      rect((i+1)*width/6-(width/12), 3.5*height/12, width/6, 3.8*width/12);
    }
    
    if (settingsToggler < 5) {
      noFill();
      stroke(255);
      rect((settingsToggler+1)*width/6-(width/12), 3.5*height/12, width/6, 3.8*width/12);
      noStroke();
    }
  ...
  if (settingsToggler == 5) {
    textFont(monoBold);
    text(">Demo Scale = " + demoScale + "<", width/2, 8.5*height/12);
  } else {  
    textFont(mono);
    text("Demo Scale = " + demoScale, width/2, 8.5*height/12);
  }
   ...
   if (settingsToggler == 6) {
    textFont(monoBold);
    text(">Friction = " + friction + "<", width/2, 9.5*height/12);
  } else {  
    textFont(mono);
    text("Friction = " + friction, width/2, 9.5*height/12);
  }
    ...
    if (key == CODED) {  
      ...
      if (settingsToggler < 4) {
        if (planetPlaceholders[settingsToggler].selected == true) {
          if (keyCode == UP) {
            planetPlaceholders[settingsToggler].mass += 10e+8;
          } else if (keyCode == DOWN) {
            planetPlaceholders[settingsToggler].mass -= 10e+8;
          }
          
          if (planetPlaceholders[settingsToggler].mass > 50e+8) {
            planetPlaceholders[settingsToggler].mass = 50e+8;
          } else if (planetPlaceholders[settingsToggler].mass < 10e+8) {
            planetPlaceholders[settingsToggler].mass = 10e+8;
          }
        }
      } 
...
class Planet {
  ...
  Planet(PVector l, float _mass, color _colour){
    pos = l.get();
     mass = _mass;
     colour = _colour;
  }
...
class Particle {
  ...
  void applyForce() {
    ...
    for (int i = 0; i < planets.size() ; i++) {
      ...
      double g =  planets.get(i).mass * this.mass / (10e+7+distSq);
  ...
  void updateStatus(double _distSq, int _i) {
...
class planetPlaceholder {
  ...
  planetPlaceholder(color _colour, boolean _selected) {
    selected = _selected;
    colour = _colour;    
    mass = 10e+8;
  }
